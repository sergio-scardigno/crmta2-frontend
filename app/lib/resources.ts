...
export async function updateTenant(id: number, data: Partial<{ name: string; is_active: boolean }>) {
...
export async function createPrintFromCalculation(
  printData: Omit<CreatePrintData, 'costo_maquinas_usd' | 'costo_trabajadores_usd' | 'costo_materiales_usd' | 'costo_desperdicio_usd' | 'costo_total_usd' | 'costo_unitario_usd' | 'costo_sugerido_total_usd' | 'costo_sugerido_unitario_usd' | 'costo_sugerido_total_local' | 'costo_sugerido_unitario_local' | 'valor_dolar' | 'tarifa_mano_obra_usd_h' | 'costo_labor_usd' | 'minimo_trabajo_ars' | 'precio_calculado_ars' | 'precio_final_ars'>,
  costBreakdown: CostBreakdown & { tarifa_mano_obra_usd_h?: number; minimo_trabajo_ars?: number },
  valorDolar: number
) {
  const fullPrintData: CreatePrintData = {
    ...
    costo_sugerido_unitario_local: costBreakdown.costo_sugerido_unitario_local,
    ...
    precio_calculado_ars: (costBreakdown as any).precio_calculado_ars ?? costBreakdown.costo_sugerido_total_local,
    precio_final_ars: (costBreakdown as any).precio_final_ars ?? costBreakdown.costo_sugerido_total_local,
...
export async function updateFixedExpense(id: number, data: UpdateFixedExpenseData) {
...
export async function fetchFixedExpensesMonth(categories: string[] = ["luz", "agua"]) {
  const params = new URLSearchParams();
  categories.forEach(cat => params.append("categories", cat));
  return api.get<FixedExpense[]>(`/fixed-expenses/month?${params.toString()}`);
}
